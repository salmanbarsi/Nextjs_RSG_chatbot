...
const Inputarea = ({ isDarkMode, setInputMessage, inputMessage, isLoading, handleKeyPress, handleSendMessage, status, stop}: any) => {
        ...
        <div className={`mb-4 p-3 rounded-lg border ${isDarkMode ? 'bg-gray-700/50 border-gray-600' : 'bg-gray-50 border-gray-200'}`}>
          <div className="flex flex-wrap gap-2">
            {selectedFiles.map((file, index) => (
              <div key={index} className={`flex items-center space-x-2 px-3 py-2 rounded-lg border ${ isDarkMode ? 'bg-gray-600 border-gray-500 text-gray-200' : 'bg-white border-gray-300 text-gray-700'}`} >
                ...
                <button onClick={() => handleRemoveFile(index)} className={`p-1 rounded-full hover:bg-opacity-20 transition-colors ${ isDarkMode ? 'hover:bg-red-400 text-gray-300' : 'hover:bg-red-500 text-gray-500' }`} >
                  <X className="w-3 h-3" />
                </button>
              </div>
            ))}
